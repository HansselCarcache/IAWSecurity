 ...
 */
@WebServlet("/Sl_GuardarFotoPerfilF")
public class Sl_GuardarFotoPerfilF extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Sl_GuardarFotoPerfilF() {
        super();
				...
				if(!uploaded.isFormField()) {
					...
					if(formatos.contains(uploaded.getContentType())) {
						...
						if(dtu.guardarFotoUser(Integer.parseInt(idusuario), url)) {
							response.sendRedirect("production/InicioFacilitador.jsp?msj=1");
						}
						else {
							response.sendRedirect("production/InicioFacilitador.jsp?msj=2");
						}
					}
				}
			}
