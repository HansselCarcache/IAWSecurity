...
@WebServlet("/Sl_Docente")

public class Sl_Docente extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(ngu.existeIdUCA(tus.getId_uca())) {
				response.sendRedirect("registrarNuevoDocente.jsp?msj=1");
			}else if(ngu.existeCedula(tus.getCedula())) {
				response.sendRedirect("registrarNuevoDocente.jsp?msj=2");
			}else
			...
			if(tus2.getId_user()>0) {
				...
				if(dtus2.guardarUser(tus2) && dtur.guardarRoluser(tur)) {
					...
					response.sendRedirect("docenteNoRegistrado.jsp");
				}
			}
