...
public class Ng_Usuario {
	...
	public boolean accesoDesarrollador(int id_rol) {
		boolean existe = false;
		try {
			c = poolConexion.getConnection();
			ps = c.prepareStatement("SELECT * FROM gestion_docente.vw_rol_opciones where id_rol=? and nombre_opcion = 'InicioDesarrollador.jsp'", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY, ResultSet.HOLD_CURSORS_OVER_COMMIT);
			ps.setInt(1, id_rol);
			rs = ps.executeQuery();
			if(rs.next()) {
				existe = true;
			...
			System.out.println("DATOS ERROR accesoDesarrollador(): "+ e.getMessage());
			e.printStackTrace();
		}
		finally {
			try {
